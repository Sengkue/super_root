<template>
    <div class="w-full">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <div class="relative w-full sm:w-64">
          ...
          <input 
            ...
            class="w-full bg-slate-800 border border-slate-700 text-slate-200 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block pl-10 p-2.5 transition-colors"
          >
        </div>
      </div>
      ...
      <div v-else-if="filteredUsers.length === 0" class="text-center py-12 text-slate-400 bg-slate-800 rounded-2xl border border-slate-700">
        <span class="text-5xl block mb-4">📭</span>
        <p class="text-xl">{{ searchQuery ? 'No users match your search.' : 'No other users found.' }}</p>
      </div>

      <div v-else class="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div v-for="user in filteredUsers" :key="user.id" class="bg-slate-800 rounded-xl border border-slate-700 shadow-sm overflow-hidden hover:border-slate-600 transition-colors flex flex-col">
          <!-- Cover Photo area -->
          ...
          <div class="px-4 pb-4 flex-1 flex flex-col relative">
            ...
            <div class="flex flex-col gap-2 mt-auto pt-3 border-t border-slate-700/50">
              ...
              <button @click="toggleFollow(user)" class="w-full text-white py-1.5 px-2 rounded-lg font-bold text-xs flex items-center justify-center gap-2 transition-colors" :class="user.isFollowing ? 'bg-slate-600 hover:bg-slate-700' : 'bg-indigo-600 hover:bg-indigo-700'">
